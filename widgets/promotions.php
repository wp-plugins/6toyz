<?php
	class PromotionsSixtoys extends WP_Widget {
		
		/**
		 * Constructeur :
		 */
		function PromotionsSixtoys() {
			$widget_ops = array('classname' => 'PromotionsSixtoys', 'description' => 'Affiche les promotions de votre site 6toyz');
			$this->WP_Widget('PromotionsSixtoys', 'Promotions 6toyz', $widget_ops);
		}
		...
		function widget($args, $instance) {
			...
			if (get_option('flux', '') != '') {
				...
				if ($category != 'all')
					$url = $flux.'/sitemap/products/output/xml/category/'.$category.'?order=promotions&limit='.$nb_products.$version;
				else
					$url = $flux.'/sitemap/products/output/xml/?order=promotions&limit='.$nb_products.$version;
				...
				echo '<ul class="promotions sixtoyz">';
	...
	add_action('widgets_init', create_function('', 'return register_widget("PromotionsSixtoys");'));
?>